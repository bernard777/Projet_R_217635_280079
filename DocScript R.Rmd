...
t1 <- t.test(MHIyes$`Total victims`,MHIyes[MHIyes$Age < 100,]$Age)
t2 <-t.test(MHIno$`Total victims`,MHIno[MHIno$Age < 100,]$Age)
t3 <-t.test(Male$`Total victims`,Male[Male$Age < 100,]$Age)
t4 <-t.test(Female$`Total victims`,Female[Female$Age < 100,]$Age)
t5 <-t.test(open$`Total victims`,open[open$Age < 100,]$Age)
t6 <-t.test(close$`Total victims`,close[close$Age < 100,]$Age)
...
Use the variables "Longitude" and "Latitude" to draw a scatterplot with the US map in background. Documente yourself to do that. Make several versions.
...
Bonus: make Choropleth maps to compare the different states.
```{r}
...
Make cross reference to other datasets, in particular datasets with information at the level of states (gun possession, death penalty, etc.).
